...
public class AreaController implements CRUDController<Area> {
	...
	public ResultMessage exportExcel(HttpServletResponse res,Area area) throws Exception {
		ResultMessage result = new ResultMessage();
		...
		areaService.exportExcel(outputStream,area);
		result.setSuccess(true);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/getFatherTree/{tid}", method = RequestMethod.GET)
	public List<Map<String, Object>> getFatherTree(@PathVariable("tid") long tid) {
		return areaService.getFatherTree(tid);
	}
