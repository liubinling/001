...
public class VehicleController implements CRUDController<Vehicle> {
	...
	public ResultMessage getVehicleByArea(Vehicle obj) {
		ResultMessage result = new ResultMessage();
		List<Vehicle> list = vehicleService.getVehicleByArea(obj);
		result.setMsg("车辆信息查询成功");
		result.setSuccess(true);
		result.setResult(list);
		return result;
	...
	public ResultMessage batchDelete(Long[] ids) {
		ResultMessage result = new ResultMessage();
		vehicleService.delBatchByIds(ids);
		result.setSuccess(true);
		result.setMsg("车辆信息删除成功！");
		return result;
	}
	...
	public ResultMessage exportExcel(HttpServletResponse res,Vehicle veh) throws Exception {
		ResultMessage result = new ResultMessage();
		...
		vehicleService.exportExcel(outputStream,veh);
		result.setSuccess(true);
